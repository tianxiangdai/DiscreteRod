...
class RigidBody:
    ...
    @njit(cache=True)
    def q_dot(t, q, u):
        q_dot = np.zeros(7, dtype=np.float64)
        q_dot[:3] = u[:3]
    ...
    @njit(cache=True)
    def q_dot_u(t, q):
        q_dot_u = np.zeros((7, 6), dtype=np.float64)
        q_dot_u[:3, :3] = np.eye(3, dtype=np.float64)
    ...
    @staticmethod
    @njit(cache=True)
    def J_P(t, q, xi=None, B_r_CP=np.zeros(3, dtype=float)):
        J_P = np.zeros((3, 6), dtype=np.float64)
        J_P[:, :3] = np.eye(3)
        J_P[:, 3:] = -Exp_SO3_quat(q[3:]) @ ax2skew(B_r_CP)
        return J_P
